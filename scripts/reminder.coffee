    ...
    if @cache.length > 0
      ...
      if @cache.length > 0
        trigger = =>
          ...
          @robot.reply reminder.msg_envelope, ''+ reminder.action
          @queue()
...
class Reminder
  constructor: (@msg_envelope, @time, @action) ->
    ...
    periods =
      ...
      minutes:
        value: 0
        regex: "minutes?|mins?|분s?"
      seconds:
        value: 0
        regex: "seconds?|secs?|초s?"
...
module.exports = (robot) ->
  ...
  robot.hear /(알림방법).*/i, (msg) ->
    msg.send "\"mrslo 알림 x분(시간,분,초) - 알려줄 내용\"... 이렇게 써주세요"

  robot.respond /알림 ((?:(?:\d+)(?:weeks?|주?|days?|일?|hours?|hrs?|시간?|minutes?|mins?|분?|seconds?|secs?|초?)[ ,]*(?:and)? +)+)- (.*)/i, (msg) ->
    time = msg.match[1]
    action = msg.match[2]
    reminder = new Reminder msg.envelope, time, action
    reminders.add reminder
    # msg.send 'I\'ll remind you to ' + action + ' on ' + reminder.dueDate()
    msg.send reminder.dueDate() + '에 "' + action + '" 알려드릴게요.'
